...
## 协程
协程并不是新的概念，早在1958年，协程的概念就被提出。协程就是一个子任务，协程的特点是非抢占式的调度，协程需要自己判断运行状况，当满足一定的条件，协程会自己主动的让出CPU给其他的协程，而进程或者线程的切换需要进入内核态交给内核处理。
对比(以linux平台为例，linux中采用轻量级进程作为线程，进程和线程唯一区别是线程之间拥有同样的内存空间)：

![线程v进程v协程](/img/post/lang/go/线程v进程v协程.png)

> 一个协程中的信息：运行栈+寄存器数值。

运行栈由程序在堆区上分配。运行栈和寄存器信息保留了协程的运行信息。寄存器的值和协程栈决定了程序当前在代码中的执行位置。
- PC： 指向了下一条执行的代码位置，可以使用jmp命令修改。
...
**协程创建、切换**        
以腾讯的 libco 协程库为例，定义了以下几个协程函数：
```
co_create  // 初始化协程的相关信息。
co_resume  // 启动一个协程        
co_yield   // 切换到下一个协程继续执行
co_release // 释放堆上协程的资源
```
协程的切换，仅仅需要改变寄存器的数值，cpu便会从需要切换的协程指定位置继续运行。协程运行的上下文保存在函数栈中。
![协程切换](/img/post/lang/go/协程切换.png)

**奇技淫巧：hook技术**         
基于 IO 多路复用的编程方式虽然提升了程序的运行效率，但是异步的编程方式降低了开发效率。为了提升编程效率，协程+hook的方式在rpc框架中得到广泛的使用，可以采用同步编程的方式实现异步编程的运行效率。
以write为例，原有的系统调用write可能会导致线程阻塞，os会切换线程；采用hook+协程的方式，write会执行以下操作：
1. 将数据发送给负责异步IO的线程；
2. 将当前协程放入协程阻塞池，从可运行的协程池中挑选一个，切换后继续执行
当IO协程返回发送数据成功或者失败后，将结果返回给协程，同时将这个协程放回可运行的协程池。

百度brpc中的bthread、腾讯libco和tars框架中的协程都采用这个思路设计协程。

C++的协程会引入新的问题，栈空间不足，以上三个框架都采用了2K栈大小，可能无法满足需求。
对于协程栈，三个rpc框架都是采用直接在堆上new出一块内存，一旦函数的调用栈超过2K，会导致程序core-dump。对于C、C++程序，允许对栈上的数据(局部变量)取指针，当栈不够的时候，如果重新分配会导致栈在内存中的地址改变，会导致程序运行出现问题。为了解决这个问题，libco的一种模式采用共享栈的方式，切换时需要将将协程栈完整的拷贝到固定的区域，切出时则完整保存栈。

协程运行在线程上：
![协程切换](/img/post/lang/go/协程运在线程上.png)

为什么协程早期没有流行起来：
计算机上不同的任务是不一样的，有的是io密集，有的需要及时响应，有的需要高吞吐，不同类型的任务做到彼此配合 很困难。开发一个任务，需要对其它任务了解。早期线程和进程数目实际不会太多，实际线程切换数目不会太多，用协程的收益不太明显。
而对于互联网服务来说，主要是微服务+高并发模式，相比于传统的应用，更加有收益。

#### goroutines
go 在语言层次实现了协程 goroutine，特点：
1. 语言层面上 hook 了阻塞的操作，阻塞操作中实现了协程切换；
2. 一个 goroutine 可以在不同的线程上切换；
3. runtime实现了 work-stealing 算法，实现协程的切换，M 个协程可以运行在N个线程上；
4. 实现了协程的抢占式调度（go1.14之后）；

#### runtime包
Go runtime是go语言的基础设施，完成任务：
- 特化语法处理
...
- P 实现了调度的局部性，一个Processor可以持有257个等待运行的协程(一个容量为 256 的队列和一个 runnext 变量，runnext 会首先被执行)；P 执行完协程后，会从继续运行自己持有的协程；
