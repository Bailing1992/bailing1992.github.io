...
## 协程
协程并不是新的概念，早在1958年，协程的概念就被提出。协程就是一个子任务，协程的特点是非抢占式的调度，协程需要自己判断运行状况，当满足一定的条件，协程会自己主动的让出CPU给其他的协程，而进程或者线程的切换需要进入内核态交给内核处理。以 Linux 为例，Linux中采用轻量级进程作为线程，进程和线程唯一区别是线程之间拥有同样的内存空间。
...
**奇技淫巧：hook技术**         
基于 IO 多路复用的编程方式虽然提升了程序的运行效率，但是异步的编程方式降低了开发效率。为了提升编程效率，协程+hook 的方式在rpc框架中得到广泛的使用，可以采用同步编程的方式实现异步编程的运行效率。
以write为例，原有的系统调用 write 可能会导致线程阻塞，os 会切换线程；采用 hook+协程 的方式，write 会执行以下操作：
1. 将数据发送给负责异步 IO 的线程；
...
3. runtime实现了 work-stealing 算法，实现协程的切换，M 个协程可以运行在 N 个线程上；
